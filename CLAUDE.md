...
> **IMPORTANT**: 이 검토·리팩토링 작업을 수행하는 주체는 **범소프트**다. **"허밍랩"·"HummingLab"은 문서·응답·커밋 메시지 어디에도 쓰지 않는다** — 예전 표기이며 재사용 금지.
>
> - 대외 제출 문서(제안서·보고서 등)의 발신·작성 주체 표기는 전부 **범소프트**로 쓴다.
> - 단, **실제 인프라 식별자는 예외다** — Linear 워크스페이스 URL(`linear.app/humminglab/...`)이나 cctv-platform GitHub org(`github.com/humminglab/...`) 같은 기존 URL·경로 자체는 실제 값이므로 바꾸지 않는다. 그 옆의 설명 라벨·소속 표기만 범소프트로 고친다.

## 이 워크스페이스의 성격

> **IMPORTANT**: `client/`·`web/`·`server/`·`device/`·`fpga/` 아래의 **`legacy/` 서브폴더**는 전부 **힐세리온(외부사) 소유 소스의 read-only 미러**다.
>
> - **`legacy/` 안은 편집·커밋·push 절대 금지.** 검토(read) 목적으로만 클론했다.
...
> - **`legacy/` 밖(컨테이너 최상위)은 우리 작업 사본이다.** 실행 계획(`docs/refactoring/r1`)이 선 저장소부터 `<컨테이너>/legacy/<repo>` 를 `<컨테이너>/<repo>` 로 복제해 쓰기 가능한 작업 사본을 만들고(각 계획의 Phase 0-0), 거기서 실제로 코드를 고친다. **`legacy/` 원본은 대조 기준선으로 그대로 둔다.**
> - **검토 산출물**(인벤토리·갭 분석·Phase 계획 문서)은 루트 git(`healcerion-platform`)의 `docs/`·`scripts/` 에 쓴다. **구현 산출물**(리팩토링된 코드)은 각 컨테이너의 작업 사본(`legacy/` 밖)에 쓴다 — 서로 다른 위치·다른 저장소다.

목적: **HLAB-2487 — 힐세리온 전체 SW 리팩토링 검토**. 기존 SW(FW·앱 등)를 cctv-platform 과 유사한 형태로 리팩토링하는 안의 타당성과 기존 대비 장단점을 검토한다. **인벤토리·갭 분석은 마무리 단계**이며, 실행 계획이 선 트랙(`docs/refactoring/r1`)부터 **구현 착수**로 넘어간다(2026-07-30~). **출시 대상은 500C·500P 로 확정됐고 belle(500L)은 범위 밖이라, belle 장비 트랙 계획(r2·r3)은 2026-08-01 삭제했다.** 계획이 아직 없는 저장소는 여전히 검토 대상이다.
...
> **IMPORTANT**: **미러 원본은 전부 `legacy/` 아래 있다.** 지금 거기 있는 모든 코드는 리팩토링의 **입력물(원본)** 이다 — 이건 실행 단계에서도 바뀌지 않는다. 컨테이너 최상위는 **실행 계획이 선 트랙부터 우리 작업 사본이 채워진다**(각 계획의 Phase 0-0).
...
**컨테이너 최상위는 트랙별로 채워진다.** 실행 계획(`docs/refactoring/r1`)이 선 저장소부터 작업 사본(`<컨테이너>/<repo>`)이 생기고, 아직 계획이 없는 저장소는 최상위가 비어 있는 것이 정상이다.
...
| 72 |`CF` | `fpga/legacy/charm-fpga` | 12 |45M | 2022-09 ~ 2022-12 | elsa/ginny 파생 Efinix FPGA. 500C 와 무관(코드로 반증, [docs/review/500c-hardware.md](docs/review/500c-hardware.md)) |
...
- **CHARM(500C/500P)** — 별도 장비 라인이고 belle 과 코드를 공유하지 않는다(Socionext 베어메탈). **단종이 아니다** — `500c-sn-fw` 최신 `FW_1_1_8_0` **2026-04-24**(Rev1.7 하드웨어·ABLIC WiFi SDK 전환), `sonex-framework` **2026-07-23** 펌웨어 굽기 실장비 검증. **범위 포함 여부는 미결**이며 상세 = [docs/review/500c-firmware.md](docs/review/500c-firmware.md)
...
- **`legacy/` 미러에는 절대 커밋·push 하지 않는다.** 커밋 대상은 루트 `healcerion-platform`(검토 산출물)과 `legacy/` 밖 작업 사본(`<컨테이너>/<repo>`, 구현 산출물) — 둘은 별도 저장소이므로 매번 어느 저장소에 커밋하는지 확인한다
