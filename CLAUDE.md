...
> - **`legacy/` 밖(컨테이너 최상위)은 우리 작업 사본이다.** 실행 계획(`docs/refactoring/r1`·`r2`·`r3`)이 선 저장소부터 `<컨테이너>/legacy/<repo>` 를 `<컨테이너>/<repo>` 로 복제해 쓰기 가능한 작업 사본을 만들고(각 계획의 Phase 0-0), 거기서 실제로 코드를 고친다. **`legacy/` 원본은 대조 기준선으로 그대로 둔다.**
...
목적: **HLAB-2487 — 힐세리온 전체 SW 리팩토링 검토**. 기존 SW(FW·앱 등)를 cctv-platform 과 유사한 형태로 리팩토링하는 안의 타당성과 기존 대비 장단점을 검토한다. **인벤토리·갭 분석은 마무리 단계**이며, 실행 계획이 선 트랙(`docs/refactoring/r1`·`r2`·`r3`)부터 **구현 착수**로 넘어간다(2026-07-30~). 계획이 아직 없는 저장소는 여전히 검토 대상이다.
...
**컨테이너 최상위는 트랙별로 채워진다.** 실행 계획(`docs/refactoring/r1`·`r2`·`r3`)이 선 저장소부터 작업 사본(`<컨테이너>/<repo>`)이 생기고, 아직 계획이 없는 저장소는 최상위가 비어 있는 것이 정상이다.
