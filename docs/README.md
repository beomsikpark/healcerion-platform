...
**같은 기능을 여러 번 적용하고 있고, 고쳤는지 자동으로 판정할 방법이 없다.**

실측이다 — Power Doppler 기능 1건이 `moana` 출하 계통 **3곳에 각각 재적용**됐고 patch-id 가 전부 다르다. 장비 완료에서 앱 출하 도달까지 **5.5개월**이다. `moana` 커밋의 **12%(712건)** 가 출하본에 없다. 상세·재현 = [review/change-cost.md](review/change-cost.md).
...
| 갈래 | 독자 | 답하는 질문 |
|---|---|---|
| **[review/](review/)** | 우리 | **지금 무엇이 있는가** — 기존 코드 실측. 다른 문서의 근거는 전부 여기로 수렴한다 |
| **[refactoring/](refactoring/)** | 우리 | **무엇을 만들 것인가 · 왜 · 어떤 순서로** — 판단 기록. 뒤집힌 판단과 그 이유를 그대로 남긴다 |
| **[proposal/](proposal/)** | **힐세리온** | **무엇을 제안하며 무엇을 요청하는가** — 대외 제출용. 내부 판단 과정은 여기 넣지 않는다 |

> **독자가 다르다.** `refactoring/` 은 우리 작업 노트라 "초기 판단은 틀렸다" 같은 정정 기록이 본문에 있다. 그것이 그대로 힐세리온에 가면 제안이 아니라 노트가 된다. 대외 문서는 `proposal/` 하나이고, 근거는 앞의 둘을 참조한다.

세 갈래 모두 **주장과 사실을 구분해 적는다** — 저장소 설명·PPT·힐세리온 자체 문서는 주장이고, 코드·해시·식별자로 확인한 것만 검증됨으로 표기한다. 미확인은 미확인으로 남긴다.
