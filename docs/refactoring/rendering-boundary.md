...
>
> **경계가 실제로 섰는지 판정하는 시험 둘** — ① SDK 단독 샘플이 ADK 없이 빌드된다(계층 분리) ② **창 없이 SDK 가 동작한다**(렌더 경계).
>
> **⚑ 정정(2026-08-02)** — 이전 판은 ②를 *"Python 에서 SDK 가 동작한다 — Python 에는 넘겨줄 창이 없으므로"* 로 적었다. **틀린 근거다.** PySide6·PyQt6·GLFW 어느 쪽이든 Python 도 네이티브 핸들을 낸다 — 실제로 §7.2 가 `sonex[qt]`(PySide6 `SonexScanWidget`)를 계획하고 있어 **같은 문서 안에 반례가 있었다.** 언어 특성(창을 못 만든다)과 사용 맥락(창을 띄우고 싶지 않다)을 뭉갠 것이다.
>
> **판정 시험 ②의 본질은 언어가 아니라 "창 없이"다.** 그리고 그 요구는 언어 범위를 좁혀도 남는다 — **CI 가 렌더 회귀를 판정할 수단이 그것뿐**이기 때문이다([goal.md §5.3](goal.md)). 드라이버가 Python 이든 C++ 이든 무관하다.
