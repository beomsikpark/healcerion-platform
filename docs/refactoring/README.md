...
> **⚑ 2026-08-02 — 목적은 그대로이나 이번 실행 범위가 뒤집혔다.** 출시 대상이 **500C·500P** 로 확정되고 UI 를 [r2](r2/plan.md)(`moana` Qt 이관)로 정하면서, **이번 범위는 제품 출시가 앞선다.** r1 은 **Qt/C++ 소비자 1종**만 완성하고 나머지 5종(C#·Python·Flutter·JNI·ObjC++)은 **연기**한다 — 폐기가 아니다. 근거·재개 조건 = [goal.md §1 ⚑·§5.2](goal.md).
