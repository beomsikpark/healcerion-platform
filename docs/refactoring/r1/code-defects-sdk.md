...
| | 판정 |
|---|---|
| **총 결함** | **SDK 21건**(치명 1 · 높음 8 · 중간 8 · 낮음 4) + **`moana` SDK 대응 계층 5건**(치명 1 · 높음 2 · 중간 2) |
...
| **컴파일러가 잡는 것** | **항목 5건 = 경고 16건.** `-Wall` 한 줄이면 드러난다 |
...
**③ 재작성은 고치기도 했고 옮겨오기도 했다.** `moana` 의 치명 결함(MO-01, 원격 힙 오버플로)은 SDK 가 **구조적으로 해소**했다 — 전선 값을 믿는 대신 잔량을 센다. 반대로 **암호 상수 3개는 글자 단위로 복사됐다**(§6). 자매 문서의 "승계 9 / 포팅 신규 17" 판정과 **방향이 같다**(반증 검증 후 값, [code-defects.md §11](./code-defects.md)).
...
**새 Phase 를 만들지 않는다.** SDK 결함 21건 중 14건이 [plan.md §4](./plan.md) 가 이미 잡아 둔 항목과 **같은 표면**에 있다.
