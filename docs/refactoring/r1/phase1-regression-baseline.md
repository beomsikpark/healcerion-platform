...
> **상태**: **1-A·1-B(재정의분)·1-C·1-D·1-E·1-E2·1-G 완료**(2026-08-02). 테스트 **101건**이 `ctest` 에서 개별 판정된다. 남은 것 = 1-F(원격 재확인, 상시) · 1-H(e2e 시나리오 — 실장비 접근 선행)
...
> **`[실행 2026-08-02]` 재정의가 옳았다는 증거가 바로 나왔다.** 대체분 두 줄을 실제로 쓰자 **장치 없이 결함 둘이 잡혔다**:
> - **프로토콜 코덱** — `putFloat`/`putDouble` 이 값 대신 **불리언 1바이트**를 썼다(XS-1/SDK-01, 이 계획 최대 결함). `getFloat` 은 4바이트를 읽으므로 **우리 코드 안에서 역함수가 아니었고**, 펌웨어를 보지 않고도 어느 쪽이 틀렸는지 확정됐다. 경계 검사의 `size_t` 언더플로(XS-4)도 같은 함수에 있었다.
> - **소켓 에코** — 논블로킹 connect 가 `select()` 쓰기 가능을 성공으로 단정해 **닫힌 포트에도 SUCCESS 를 반환했다.** `SO_ERROR` 를 보지 않았다. 인벤토리가 *"논블로킹 connect 완료 확인은 iOS 만 돼 있다"* 고 적은 것의 POSIX 쪽 실체이고, Android 도 같은 코드다.
>
> **둘 다 장치 에뮬레이터로는 오히려 못 잡았을 것이다** — 에뮬레이터는 우리가 보내는 바이트를 그대로 받도록 짜게 되고, 닫힌 포트 경로는 애초에 만들지 않는다.

> **`[2026-08-02 이어짐]` 1-G 를 넓히며 결함 둘을 더 잡았다** — 둘 다 케이스를 먼저 쓴 결과다.
> - **`String::formatted` 가 인코딩 실패에서 죽는다**(SDK-03). `vsnprintf` 가 음수를 돌려주는데 그것을 `size_t` 길이에 담아 `new char[음수+1]` 이 된다. **테스트가 실제로 재현했다**(짝 없는 서로게이트 → `Subprocess aborted`). 인벤토리가 *"실행으로 확인한 유일한 힙 오버플로 경로"* 라 적은 것을 실행으로 확인했다.
> - **`indexOf` 가 `caseSensitive` 를 받아 놓고 한 줄도 쓰지 않는다**(SDK-05). `contains` 는 이것에 위임하고 `startsWith`/`endsWith` 는 넘기지도 않았다 — **대소문자 무시를 요청해도 정확히 일치할 때만 찾았다.**
>
> **`VariantMap` 은 고치지 않고 계약만 고정했다** — SDK-02·11·14 가 한 덩어리라 따로 고치면 이중 해제가 된다(XS-2). 특히 *"`clone` 이 unsafe 포인터를 복사하지 않는다"* 는 현행 동작이 이중 해제를 피하고 있다는 뜻이고, **그 사실 자체가 XS-2 설계의 입력**이다.
