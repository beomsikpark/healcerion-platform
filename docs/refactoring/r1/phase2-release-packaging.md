...
> **상태**: 진행 중 — **2-A·2-B·2-C·2-D·2-E·2-F·2-G 완료**(2026-08-02, 커밋 `de27676d`·`564bffa8`·`74bf6d16`). `make package` 가 아카이브+체크섬을 만들고 CI 가 Debug/Release 매트릭스로 판정한다. **남은 것은 2-H(게시 대상)** — 인프라 결정 선행
...
| F-4 | **바이너리 이름 정책 적용** — 일반명 DLL 11개에 접두를 붙일지, 서브디렉토리로 격리할지 정한다. **이름을 바꾸면 앱의 수동 로드 목록**(`adk_native_methods.dart:242-295`)**이 함께 바뀐다** — 반영은 [Phase 8-C](../r1/phase8-app-migration.md) 소관이므로 이 단계에서는 결정만 한다 |
...
#### 2-실행. `[2026-08-02]` 무엇이 실제로 만들어지는가

`make package PLATFORM=linux` 실측.

| 구성(B2 8개) | 상태 |
|---|---|
| ① 네이티브 바이너리 | **8개**(`libSonexSDK.so` 포함) + 각 sha256 |
| ② 공개 헤더 | **120개** |
| ③ 의존 서드파티 | `vcpkg.json` + `THIRD_PARTY.md`. **바이너리는 넣지 않는다** — 소비자가 같은 매니페스트로 재현한다 |
| ④ 언어별 wrapper | **0 — Phase 5 산출물** |
| ⑤ 샘플 | **0 — Phase 6** 이 소스 트리 역참조를 끊은 뒤 |
| ⑥ 문서 | 2개(`BUILD.md`·`THIRD_PARTY.md`) |
| ⑦ 라이선스 고지 | **0 — 전문 수집 미구현** |
| ⑧ 버전 메타 | `package.json` — 버전·커밋·서술자·브랜치·더티 여부·빌드 시각 |

> **빈 칸을 억지로 채우지 않았다.** 매니페스트에 구성마다 **왜 비었는지**를 적는다 — 8칸을 형식만 채우면 이 패키지가 무엇을 보증하는지 알 수 없게 된다. 더티 워킹트리에서 만들면 *"이 산출물은 기록된 커밋으로 재현되지 않는다"* 고 경고한다.

**2-B 는 "만드는" 것이 아니라 "잇는" 것이었다** — `VERSION_SDK_*`(0.59.0, build 100)가 이미 있었고 git 과의 연결이 없었을 뿐이다. 그 연결을 `scripts/package.py` 가 한다.

**2-C 에서 나온 실측이 무겁다** — `scripts/check-version.py` 가 **태그 번호와 그 커밋의 소스 상수가 어긋난 5건**을 찾았다. `v0.54.0` 은 소스가 **0.53.3** 인 커밋에 붙어 있다. 규약 밖 태그 4건(플랫폼 접미사·`-Beta`)과 합쳐, **태그를 버전의 출처로 쓸 수 없다**는 것이 수치로 확정됐다. 기존 위반은 **소급 정정하지 않는다**(이력 변경이고 0-4 선행) — 앞으로 붙는 태그만 error 로 막는다.
