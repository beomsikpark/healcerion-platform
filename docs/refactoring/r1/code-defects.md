...
| **계보** | 승계 **9** · **포팅 신규 17** · `moana` 고유 3 · 혼합 1(S-5) · **미판정 9** — **2026-08-02 반증 검증에서 S-1 이 회귀 → 승계로 바뀌었다**(§11) |
| **가장 무거운 축** | **데이터 파괴 3건**(D-2·D-3·D-4) — 전부 조용히 일어나고 사용자에게 오류가 표시되지 않는다. **D-1 은 검증에서 기능 결손으로 재분류**됐다(§11) |
| **`moana` 폐기로 해소되는 것** | **3건뿐**(`moana` 고유). 승계 **9건**은 ADK 에 그대로 남는다 |

**포팅 신규 17건은 나쁜 소식이 아니라 좋은 소식이다.** `moana` 원본에 **정답이 이미 있다**는 뜻이므로, 설계를 다시 하는 게 아니라 **원본 한 줄을 옮겨오면 끝나는 작업**이다. 실제로 D-2 는 하드코딩 문자열을 변수로 바꾸는 것이고, F-4 는 주석 처리된 한 줄을 되살리는 것이다. **17건 중 대부분이 이 성격이다.** 다만 **S-1(평문 HTTP)은 여기서 빠진다** — 검증 결과 `moana` 도 같은 평문 URL 이라 원본에 정답이 없다(§11.1).

**이것이 [plan.md §8](./plan.md) 의 *"기존 동작 보존"* 에 단서 하나를 붙인다** — 보존 기준선은 **ADK 의 현재 동작이 아니라 `moana` 의 검증된 동작**이다. ADK 현재 동작을 그대로 고정하면 회귀 17건을 함께 굳히게 된다.

**Qt 가 가려주던 것이 드러났다.** 그 대표가 C-1 하나로, **조회 함수 9개·대입 지점 86곳**에 걸쳐 있다(2026-08-02 재계수). `QVariant::toString()` 은 NULL 컬럼에서 빈 문자열을 주지만 `std::string s = (const char*)sqlite3_column_text(...)` 는 **정의되지 않은 동작**이다. 포팅이 타입만 바꾸고 그 계약 차이를 메우지 않았다.
...
| # | 결함 | 위치 | 계보 |
|---|---|---|---|
| **D-1** | **`UPDATE` 에 `WHERE` 절이 없다** — `UPDATE DeviceData SET Serial=…, IsStolen=…;` 가 전 행에 걸린다. 서버에서 장비 정보를 받을 때마다 실행된다. **⚠ 영향 정정(2026-08-02 반증 검증)**: `DeviceData` 는 `PRIMARY KEY('Serial')`(`:121`)이라 **등록 장비가 2대 이상이면 두 번째 행에서 UNIQUE 제약을 위반해 statement 가 통째로 abort** 된다(SQLite 기본 ON CONFLICT ABORT — 그 문장의 변경이 전부 되돌려진다). **따라서 다른 장비 행이 덮어써지지 않는다 — 갱신이 조용히 실패할 뿐이다.** 1대만 등록된 상태에서는 우연히 정상 동작한다. **결함은 실재하나 성격은 데이터 파괴가 아니라 기능 결손**이다 | ADK `HCDataBaseAdapter.cpp:1502-1512` · `moana` `SononDataBaseAdapter.cpp:1315-1322`(WHERE 없음 동일 확인) | **승계** |
...
| # | 결함 | 위치 | 계보 |
|---|---|---|---|
| **C-1** | **NULL 컬럼 → `std::string(nullptr)`.** `reinterpret_cast<const char*>(sqlite3_column_text(...))` 를 널 검사 없이 `std::string` 에 대입하는 곳이 **9개 조회 함수·86개 지점**에 있다(보호된 `val ? … : ""` 는 25곳)(`getAppSettings`·`getDeviceList`·`getAllPatient`·`getAllWorkItemInfo`·`getPatientInfo`·`getSnapshotInfoBySnapshotID`·`getStudyInfoByStudyID`·`getUserAppLog`·`getServerAppLog`). `AppSetting`·`PatientInfo` 의 대부분 컬럼에 `NOT NULL` 제약이 없어 정상 데이터로 트리거된다. **같은 파일의 `getPatientInfoDbFromQueryResult` 는 `val ? ... : ""` 로 제대로 막고 있다** — 방어가 절반만 적용됐다. **⚠ 촉발 경로 보강(2026-08-02 반증 검증)**: NOT NULL 부재보다 확실한 경로가 코드 안에 있다 — `AddColumnIfNotExist(strAppSetting,"Occupation"/"SerialNumber","TEXT")`(`:210-211`)가 **`DEFAULT` 없이** 컬럼을 추가해 **기존 행을 전부 NULL 로 만들고**, `:920-921` 이 그 두 컬럼을 비보호로 읽는다 → **구버전 DB 사용자는 업그레이드 직후 확정 크래시** | ADK `HCDataBaseAdapter.cpp` 다수(`:210-211`·`:920-921`) | **포팅 신규**(`moana` 는 `QVariant::toString()` 으로 널 안전) |
...
| **C-7** | **⚠ 메커니즘 정정(2026-08-02 반증 검증).** `memset(buff, 0, sizeof(*buff))` 가 **1바이트만 0으로 만드는 것은 맞으나, 그 1바이트가 `buff[0]` 이라 `strcat` 은 offset 0 부터 올바르게 이어붙인다** — *"쓰레기 값 안의 널 위치부터"* 는 성립하지 않는다. **실제 결함은 크기 계산이다**: 할당 `4·size` 대비 기록량이 `3·size + 2`(항목 3자 × size + `\n` + NUL)라 **`size ≤ 1` 에서 넘친다**(size=1 → 5 > 4, size=0 → `new char[0]` 에 즉시 1바이트 memset). 게다가 **유일한 출력 `// qDebug() << buff;` 가 주석 처리돼 있어, 이 함수는 아무 일도 하지 않으면서 그 위험만 남긴다** | `moana` `SononUtils.cpp:219-231` | **`moana` 고유** |
| **C-8** | **널 역참조.** `gethostbyname(host)->h_addr` 를 반환값 검사 없이 역참조 — DNS 실패 시 즉시 크래시 | `moana` `WindowFtpUpload.cpp:99` | **`moana` 고유** |
| **C-9** | **고정 버퍼 오버플로.** `char latestActivationKey[32]`·`char presetName[32]` 에 `sprintf(..., "%s", 외부문자열)`. CVIE 활성화 키·프리셋 이름은 설정/서버에서 오는 외부 입력이다 | `moana` **`framework/ContextVision/ContextVision.cpp`**`:307,354,448` (선언 `.h:59,60`) — 경로 정정(2026-08-02, `ImageProc/` 아님) | **`moana` 고유** |
...
| # | 결함 | 위치 | 계보 |
|---|---|---|---|
| **S-1** | **클라우드 API 전체가 평문 HTTP.** `base_url = "http://sonex.healcerion.com:8080/API/"` — 로그인·회원가입·비밀번호 변경·프로필 조회가 전부 이 경로다. **⚠ 계보 판정 정정(2026-08-02 반증 검증) — 회귀가 아니라 승계다.** 이전 판은 `moana` `SononCloud.cpp` 가 전부 `https://` 인 것을 근거로 회귀라고 적었으나, **그 파일은 Firebase Functions·펌웨어 다운로드 URL 을 담은 다른 파일**이다. 실제 클라우드 API 클라이언트는 **`SononCloudApi.cpp:28` 이고 거기 `baseUrl` 이 `"http://sonex.healcerion.com:8080/API/"` — ADK 와 글자 단위로 같다**(`NetworkMonitoringThread.cpp:27,30` 도 같은 평문 호스트). **HTTPS→HTTP 전환은 일어난 적이 없다** | ADK `HCNetworkProcess.cpp:17` · `moana` `framework/Network/SononCloudApi.cpp:28`(동일) | **승계** |
| **S-2** | **평문 비밀번호가 로그에 남는다.** `CHANGE_PASSWORD` 는 **해시하기 전에** `paramMap` 전체를 순회 출력한다(`cur_pw`·`new_pw` 평문). **⚠ 범위 보강(2026-08-02 반증 검증)**: `CHANGE_PASSWORD` 전용이 아니다 — **`handleEvent` 진입부(`:90-93`)가 명령을 가리지 않고 `paramMap` 전체를 `key = value` 로 출력**하므로 **로그인·회원가입·탈퇴의 비밀번호도 같은 경로로 남는다.** `moana` 에는 대응 로그가 없다 | ADK `HCNetworkProcess.cpp:90-93`(전 명령) · `:333-337`(CHANGE_PASSWORD) | **포팅 신규** |
...
| # | 결함 | 위치 | 계보 |
|---|---|---|---|
| **L-1** | 파일 변환 실패 시 `return -20000` 으로 즉시 빠져나가 **zip writer 를 닫지도 해제하지도 않는다.** 열린 핸들과 부분 zip 파일이 남는다. **탈출 지점이 1곳이 아니라 3곳이다**(2026-08-02 재확인) — writer 는 `:217` 에서 만들어지고 `close`·`delete` 는 `:285`·`:292` 인데 그 사이에서 `return -20000` 이 세 번 나온다 | ADK `ZipUtil.cpp:245 · 600 · 687` | 미판정 |
...
| **L-4** | **널 종료 없는 `strncpy`.** 인코딩 변환 블록의 `strncpy(utf8_name, utf8_string, sizeof(utf8_name))` 가 종료문자 자리를 남기지 않고 수동 종료도 하지 않는다 — 같은 함수 앞쪽(`:647-648`)은 `-1` 후 수동 종료를 한다. 파일명이 버퍼를 정확히 채우면 이후 `mz_path_resolve` 가 버퍼 밖을 읽는다. **⚠ 범위 정정(2026-08-02)**: 분기 기준이 `#if defined(_MSC_VER)` 이므로 *"iOS 해제 경로만"* 이 아니라 **MSVC 아닌 전 플랫폼**(Android·iOS·macOS·Linux)이다 | ADK `ZipUtil.cpp:660` · `moana` `ZipUtil.cpp:421` | **승계** |
...
| 항목 | 내용 | 선행 |
|---|---|---|
| **X-1** | **데이터 파괴 3건 정지** — D-2(테이블명)·D-3(부분 UPSERT)·D-4(파일 잔존). **D-1(WHERE 절 부재)은 같은 자리에서 함께 고치되 성격은 기능 결손**이다(§11). 넷 다 수정 자체는 국소적이나 **이미 손상된 데이터의 복구 여부는 별개 판단**이다(힐세리온 확인 필요) | 없음 |
| **X-2** | **널 안전성 일괄 정정** — C-1 의 9개 함수. 같은 파일에 이미 있는 `val ? ... : ""` 패턴을 전면 적용하거나, 조회 헬퍼 하나로 감싼다 | 없음 |
| **X-3** | **전송·로그 보안 3건** — S-1(HTTPS 전환, 서버 측 지원 확인 필요)·S-2(비밀번호 로그 제거, **전 명령 경로**)·S-3(진단 파일 제거). **S-1 은 `moana` 도 같은 평문 URL 이라 원본 참조로는 답이 안 나온다** — 백엔드가 TLS 를 제공하는지가 유일한 선행 조건이다(§11) | S-1 은 백엔드 확인 |
...
| **X-6** | **포팅 회귀 17건을 원본 참조로 정정** — 각 건마다 `moana` 원본이 정답이므로 **설계 판단 없이 옮겨오면 된다.** 이 축에서 가장 값싼 항목이다. **의도된 변경인지 확인이 필요한 것은 S-1 하나뿐**(HTTPS→HTTP 가 서버 구성 때문인지) | 없음(S-1 만 백엔드 확인) |
...
| **ADK 결함이 새 앱의 결함이 된다** | **그렇다** — r2 는 `moana/app/` 을 ADK 위에 얹는다. **[r2 Phase 4](../r2/plan.md)(`*Db` 337회 · `CDataManager` 256회 → ADK)의 목적지가 §3 데이터 파괴 3건이 있는 계층**이다. X-1 이 그 전에 끝나야 한다 |
...
## 11. 반증 검증 (2026-08-02, 별도 세션)

**39건 전수를 "결론을 반증하려는 관점"으로 코드에서 다시 읽었다.** 검증자는 이 문서를 쓰지 않은 쪽이고, 각 항목마다 *"이 서술이 틀렸다면 어디서 틀렸겠는가"* 를 먼저 세운 뒤 원본을 열었다.

| 판정 | 건수 |
|---|---:|
| **확정** — 서술·위치·영향이 전부 코드와 일치 | **31** |
| **정정** — 결함은 실재하나 서술이 틀렸다 | **5** |
| **보강** — 문서보다 실제가 더 나쁘다 | **3** |
| **반증** — 결함 자체가 성립하지 않음 | **0** |

**결함이 없다고 뒤집힌 항목은 없다.** 39건 전부 코드에 실재한다. 바뀐 것은 **하나의 계보 판정과 네 개의 서술**이다.

### 11.1 정정 5건

| # | 무엇이 틀렸나 | 근거 |
|---|---|---|
| **S-1** | **계보가 회귀가 아니라 승계다.** 이전 판은 `moana` `SononCloud.cpp` 가 전부 `https://` 인 것을 근거로 "포팅 신규(회귀)"라 했으나 **그 파일은 Firebase·다운로드 URL 파일이고 클라우드 API 클라이언트가 아니다.** 실제 클라이언트 `SononCloudApi.cpp:28` 의 `baseUrl` 이 ADK 와 **글자 단위로 같은 평문 URL** 이다 | `git show origin/service_QT693:framework/Network/SononCloudApi.cpp` `:28` · `NetworkMonitoringThread.cpp:27,30` |
| **D-1** | **데이터 파괴가 아니다.** `DeviceData` 는 `PRIMARY KEY('Serial')` 이라 WHERE 없는 UPDATE 는 2행 이상에서 **UNIQUE 위반 → statement abort** 로 끝난다. 다른 장비 행이 덮어써지지 않고 **갱신이 조용히 실패**한다. 결함은 실재하나 **기능 결손**이다 | `HCDataBaseAdapter.cpp:121`(DDL) · `:1502-1512` |
| **C-7** | **메커니즘이 틀렸다.** `memset(…, sizeof(*buff))` 가 0으로 만드는 1바이트는 **`buff[0]`** 이라 `strcat` 은 offset 0 부터 정상 동작한다. 실제 결함은 **크기 계산**(할당 `4·size` vs 기록 `3·size+2` → `size ≤ 1` 에서 초과)이고, 게다가 **유일한 출력이 주석 처리**돼 있다 | `SononUtils.cpp:219-231` |
| **L-4** | **범위가 틀렸다.** 분기 기준이 `#if defined(_MSC_VER)` 이므로 "iOS 해제 경로만"이 아니라 **MSVC 아닌 전 플랫폼**(Android·iOS·macOS·Linux)이다 | `ZipUtil.cpp:645-660` |
| **C-9** | **경로가 틀렸다.** `framework/ImageProc/` 아님 → **`framework/ContextVision/`** | `git ls-tree origin/service_QT693` |

**S-1 의 정정이 계획에 미치는 영향**: §9.2 **X-6 의 질의 목록에서 S-1 이 빠진다.** *"HTTPS→HTTP 전환이 의도된 것인지 확인"* 은 **전환이 없었으므로 성립하지 않는 질문**이다. S-1 은 원본 참조로 답이 나오지 않으며, **백엔드가 TLS 를 제공하는지**만이 선행 조건이다.

### 11.2 보강 3건 — 문서보다 실제가 더 나쁘다

| # | 보강 내용 |
|---|---|
| **C-1** | 비보호 대입이 "9개 함수"가 아니라 **86곳**(보호된 것 25곳). 더 중요한 것은 **촉발 경로가 추정이 아니라 확정**이라는 점 — `AddColumnIfNotExist(strAppSetting,"Occupation"/"SerialNumber","TEXT")`(`:210-211`)가 **`DEFAULT` 없이** 컬럼을 추가해 기존 행을 전부 NULL 로 만들고, `:920-921` 이 그 두 컬럼을 비보호로 읽는다. **구버전 DB 사용자는 업그레이드 직후 확정 크래시** |
| **S-2** | `CHANGE_PASSWORD` 전용이 아니다. **`handleEvent` 진입부(`:90-93`)가 명령을 가리지 않고 `paramMap` 전체를 출력**하므로 로그인·회원가입·탈퇴 비밀번호도 같은 경로로 남는다 |
| **L-1** | 탈출 지점이 1곳이 아니라 **3곳**(`:245`·`:600`·`:687`). writer 는 `:217` 생성, `close`/`delete` 는 `:285`/`:292` |

### 11.3 SDK 축과 맞물리는 것

**C-10**(`Log::debugf` 의 `%s` 2개에 인자 1개)은 **단독 UB 로 끝나지 않는다.** 그 로그는 `String::formattedv` 를 거치는데, 그 함수가 **`vsnprintf` 의 음수 반환을 검사하지 않아 `size_t` 언더플로 → 0바이트 할당 → 무제한 기록**으로 이어진다([code-defects-sdk.md](./code-defects-sdk.md) SDK-03). 즉 **인자 부족이 쓰레기 포인터를 `%s` 에 물리면 그대로 힙 오버플로 경로**다. 두 결함은 따로 고치면 안 되고, **SDK-03 을 먼저 막아야 C-10 의 최악이 사라진다.**

### 11.4 검증에서 새로 나온 것

- **`hc_createDatabaseHelperInstance`·`hc_destroyDatabaseHelperInstance` 가 `#if OS_WINDOWS / #elif OS_ANDROID / #endif` 로만 정의돼 iOS·macOS 에는 심볼이 아예 없다**(`HCDatabaseHelper.cpp:816-838`). C-6 과 같은 블록이며, [plan.md](./plan.md) 판정 11(바인딩 정합성)의 ADK 쪽 사례다.

### 11.5 이 검증이 하지 않은 것

- **여전히 정적 판독이다.** 실행 재현은 없다 — §10 의 한계가 그대로 유효하다.
- **미판정 9건의 계보는 그대로 미판정**이다. 이번 검증은 **서술의 정확성**을 본 것이지 대조하지 않은 원본을 새로 대조한 것이 아니다(S-1·D-1·D-3·F-4 는 예외로 원본을 열었다).
- **`moana/app/` 은 이번에도 보지 않았다.** §10 의 가장 큰 공백은 해소되지 않았다.
...
- [../r2/plan.md](../r2/plan.md) — `moana` UI → SDK/ADK 이관. **§2.3**(무엇이 남고 사라지는가) 이 `moana` 고유 3건의 처리 근거 · **Phase 4** 가 데이터 파괴 3건의 소비처
