...
**이 문서가 답하는 질문: 지금 코드에 동작 결함이 있는가, 있다면 그것은 리팩토링으로 없어지는가.**
...
**포팅 신규가 승계의 두 배가 넘는다.** ADK 는 `moana` 결함을 물려받았을 뿐 아니라 **옮기는 과정에서 그보다 훨씬 많이 만들었다**. 이것이 이 검토의 핵심 결과이고, [plan.md §8](./plan.md) 이 전제한 *"기존 동작 보존"* 이 **어느 기준선을 보존한다는 뜻인지** 되묻게 만든다 — ADK 의 현재 동작을 보존하면 `moana` 대비 회귀 18건을 함께 고정하게 된다.
...
| **X-6** | **`moana` 대비 회귀 18건 판정** — 포팅 신규 항목마다 `moana` 원본 동작이 정답이다. **어느 쪽이 의도인지 힐세리온에 확인**한 뒤 정정한다(특히 S-1 HTTPS→HTTP 전환이 의도된 것인지) | 힐세리온 회신 |
