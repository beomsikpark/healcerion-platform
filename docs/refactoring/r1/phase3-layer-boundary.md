...
> **상태**: 진행 중 — **3-A·3-B·3-F·3-H(모듈 로딩 축)·3-K 완료 · 3-I 부분 완료**(2026-08-03). SDK→ADK 역방향 참조 **15→0** 이고 `adk/` 를 지운 트리에서 SDK 8개 모듈이 빌드된다. 3-C·3-E(실측)·3-G 는 판정 장치·실측 완료(2026-08-02, 커밋 `b8dc55bf`·`ce121c4a`·`b83869f5`). 코드 이동(3-A·3-B·3-D·3-E·3-H·3-I·3-J·3-K)은 미시작
...
#### 3-H 실측·결과 — **모듈 로딩이 가장 큰 축이었다** `[2026-08-03]`
...
모듈 6개의 로더가 다른 것은 **넷뿐**이다 — dll/so 이름 · 생성자 심볼 · 담을 멤버 포인터 · 로그 라벨. 그 넷을 `ModuleSpec` 자료로 두고 흐름을 하나로 모았다.

```
HCSonexSDK.cpp  1,875 → 1,453줄   ·   플랫폼 분기 12곳 → 2곳
```

**공개 서명은 그대로다**(H-2) — `SonexSDK::load*Library` 는 남아 있고 몸통만 위임한다. 공개 심볼 59개 변화 없음.

> **흩어진 분기를 줄이는 것이 요점이다.** r1 에서 같은 형태의 목록이 여러 곳에 흩어져 있어 세 번 죽었다 — [4-A A-2](./phase4-render-boundary.md) 의 EGL 분기 3곳, [4-C](./phase4-render-boundary.md) 의 로더 14곳, 그리고 그 14곳을 고치던 우리 자신.

**합치면서 Android 결함 하나가 드러났다.** `loadImageFilterLibrary` 의 Android 분기가 이 모듈의 핸들을 **ScanBuffer 의 핸들 슬롯에** 넣고 있었다(다른 5개는 전부 자기 것에 넣는다).

| 결과 | |
|---|---|
| `imageFilterHinst` 가 계속 `nullptr` | free 가 `NOT_PREPARED` 로 빠져 `libImageFilter.so` 가 닫히지 않고 `preFilter`·`postFilter` 가 소멸되지 않는다 |
| ScanBuffer 의 핸들이 덮인다 | free 가 **다른 라이브러리**에서 소멸자 심볼을 찾고, 못 찾은 채 그 라이브러리를 `dlclose` 한다 |

**모듈마다 자기 핸들을 넘기는 구조가 되면 이 실수 자체가 불가능하다** — 12개 로더 중 남의 핸들을 만지는 것 0으로 확인했다.
...
| # | 작업 |
|---|---|
| I-1 | ✅ **완료(2026-08-03)** — 17 case · 57줄 → 표 39줄. 전부 `return parseX(requestCode, json)` 이라 1:1 이었다 |
| I-2 | ✅ **완료(2026-08-03)** — 40 case · 122줄 → 표 56줄. 폴스루 4연속(I-5 가 지목한 자리)은 **같은 핸들러를 가리키는 4행**이 됐다 — 암묵적 순서 의존이 명시적 매핑으로 바뀐다 |
...
| **I-6** | ✅ **완료(2026-08-03)** — `InstructionSet{300C,300L,500C,500L,500P,Default}` 6벌의 `parseReceivedPacket` 진입부를 기반 클래스 `readValidPacketHeader` 한 벌로 모았다(진입부 40~49줄 × 6 → 9줄 × 6). 죽은 비교(`skip < 0`·`contentSize < 0`, 둘 다 `size_t` 라 항상 거짓)를 걷어내 **`-Wtype-limits` 13 → 1**, 전체 경고 242 → 230 |
| **I-7** | ✅ **지켰고, 그 결과 계획이 틀린 것으로 드러났다** — §I-6-실측 참조 |

#### I-6-실측 — **계획의 주장이 반증됐다** `[2026-08-03]`

계획은 이렇게 적었다: *"`size_t skip = current() - 2; if (skip < 0)` 는 항상 거짓이라 1바이트만 수신된 상태(TCP 분할 시 흔하다)에서 `skip = SIZE_MAX` 가 `outLength` 로 나가고 `pop()` 이 거부돼 그 바이트가 링에서 소비되지 않는다."*

**재현되지 않는다.** 진입부의 `length < HC_PACKET_HEADER_SIZE` 가드가 먼저 막아 `current()` 가 2 보다 작아지는 입력이 그 줄에 도달하지 못한다. `test/device/test_packet_header_validation.cpp` 의 8케이스가 0~13바이트 전 구간·접두 없는 버퍼·`'H'` 반복 버퍼를 돌며 확인했다 — 전부 `outLength` 0 또는 버퍼 크기 이내다.

**I-7 이 "동작을 바꾸기 전에 경계값 케이스를 먼저 쓰라" 고 한 것을 지켰더니, 고칠 근거가 아니라 계획을 고칠 근거가 나왔다.** 이 절이 그 정정이다.

죽은 비교를 지우는 이유는 그래서 다른 데 있다 — **읽는 사람이 검증이 있다고 믿는다.** 실제로 살아 있는 검사는 `targetId`·`sessionId` 둘뿐이고, 코드에 `// FIXME: Check header validation` 이 붙어 있는 것이 그 방증이다.

**동작이 하나 바뀐다 — `Default`.** 그것만 `hasAmountOfData(contentSize)` 검사가 없어 본문이 덜 온 상태에서도 `readCommand()` 로 넘어갔다. 나머지 5개 모델은 전부 그 검사를 한다. 같은 프로토콜이므로 빠진 쪽이 맞춰지는 것이 옳다고 판단했고, **같은 입력을 500C 와 Default 에 각각 돌려 결과가 같은지 보는 케이스 3건**이 그 변경을 판정한다.

> **3-F 의 빈틈이 여기서 드러났다.** 이 작업에서 `sdk/include/HCInstructionSet.h` 만 고쳤더니 빌드가 깨졌다 — 내부 사본이 따로 있었다. 3-F 는 **갈라진** 49벌만 다뤘고 md5 가 같은 **71벌은 그대로 뒀는데, 그것들도 편집 한 번이면 갈라진다.** 게이트가 즉시 잡았고, 71벌도 전부 전달 헤더로 바꿨다(동일 0 · 전달 122 · 갈림 0). **완료 조건을 "갈림 0" 으로 잡은 것이 부족했고 "사본 0" 이어야 한다.**

#### I-1·I-2 결과 — **게이트가 눈이 멀 뻔했다** `[2026-08-03]`

변환 자체는 기계적이었다. 판정이 둘로 섰다.

1. **쌍 대조** — 변환 전후 (코드 → 핸들러) 쌍을 소스에서 뽑아 비교했다. 17/17 · 40/40 동일, 어느 쪽에만 있는 것 0.
2. **기준선** — `check-dispatch.py` 의 기준선은 **변환 전에 기록된 것**인데 변환 후 `dispatch sets unchanged (23 functions, 458 cases)` 가 나온다.

> **변환 직후 게이트는 "dispatch function gone" 만 보고했다.** `case` 만 세고 표는 못 세기 때문이다 — **그 변환을 지키라고 만든 게이트가 그 변환 때문에 무력해진 셈**이다. 표도 세도록 고쳤고, 표에서 한 줄을 지워 잡히는 것을 확인했다.
>
> 3-F 에서 "동일 사본" 을 안 센 것과 같은 종류다. **게이트가 형태를 세면 형태가 바뀔 때 무력해진다. 세야 하는 것은 형태가 아니라 대상이다.**

#### I-3-실측 — **형태가 달라 같은 변환이 성립하지 않는다** `[2026-08-03]`

```
case REQUEST_SCAN_MODE_B:
    createPacketScanStart(out, 2);
    res = createPacketParamB(out, param);
    if (res == SUCCESS) res = createPacketWriteBFunc(out, param);
    if (res == SUCCESS) res = createPacketScanStart(out, 1);
    break;
```

I-1·I-2 는 모든 case 가 단순 위임이라 표가 곧 같은 뜻이었다. I-3 은 **다단계 시퀀스에 오류 전파**가 붙어 있어, 표로 옮기면 순서·오류 전파가 조용히 바뀔 수 있고 그것은 **장비로만 드러난다.**

그래서 **형태를 바꾸기 전에 판정 수단부터 세웠다.** `getInstructionPacket` 은 장비가 필요 없다 — 명령·파라미터로 패킷 바이트를 만드는 순수 함수다(`test/device/test_instruction_golden.cpp`, 4케이스).

세우는 과정에서 둘이 나왔다.

| 발견 | 내용 |
|---|---|
| **`CMD_SCANNER_TIME_SYNC` 는 순수 함수가 아니다** | 벽시계를 패킷에 넣는다. 빼지 않으면 순수성 케이스가 **초 경계에서만 가끔 깨진다.** 그 명령은 시각을 주입할 수 있게 되기 전에는 바이트로 판정할 수 없다 |
| **I-3 의 전제가 부분적으로만 맞다** | 파라미터 없는 명령 12개 중 **7개가 500C·500P·500L 에서 바이트까지 동일**하고 5개가 다르다. *"모델별 차이가 실제 프로토콜 차이"* 라 했으나 **절반 이상은 실제로 같다** — 공통화 여부는 모델 단위가 아니라 **명령 단위**로 판단해야 한다 |

#### 3-A 실측 — **넷 중 셋은 존재한 적 없는 경로였다** `[2026-08-03]`

계획은 `adk/library` 참조를 14건으로 봤고 실측은 15건인데, **성격이 계획과 다르다.**

| 참조 대상 | 건수 | 실재 |
|---|---:|---|
| `adk/library/openssl-1.1.1d_ios` | 6 | ✅ 있다(14M·106파일) |
| `adk/library/angle_ios` | 4 | ❌ **저장소 어디에도 없다** |
| `adk/library/opencv_3.4.6_ios` | 2 | ❌ 없다 |
| `adk/library/freetype_ios` | 2 | ❌ 없다 |

즉 `sdk/sdk/Main/ios/CMakeLists.txt` 는 **커밋된 상태로 configure 되지 않는다.** 우리가 iOS 를 빌드하지 못해서 몰랐던 것이 아니라 **빌드했다면 첫 줄에서 멈췄을 것**이다. macOS 쪽도 같다(`angle`·`angle_macos`).

실재하는 것은 `sdk/third_party/ios/openssl-1.1.1d` 로 옮겼다(3-B 의 배치 규약). 나머지는 정본 위치 이름으로 바꾸되 **없는 것을 있는 것처럼 적지 않는다** — `scripts/check-thirdparty-paths.py` 가 10건을 계속 보고한다.

> **게이트를 만들며 두 번 틀렸고 둘 다 오탐이었다** — pbxproj 의 `SRCROOT` 를 한 단계 위로 잡았고, `SDK_ROOT` 를 `<repo>/sdk` 로 가정했는데 그 파일들에서는 `<repo>/sdk/sdk` 였다. **두 번째 오탐을 믿고 경로를 고쳤다가 멀쩡한 경로를 깨뜨렸다.** 게이트가 틀리면 그것을 고치는 손이 함께 틀린다.

#### 3-K 결과 — **`adk/` 를 지운 트리에서 SDK 가 빌드된다** `[2026-08-03]`

K-2 의 판정 조건 그대로 실행했다. 결과 **8개 모듈 빌드 성공**(`scripts/check-sdk-only.sh`).

grep 보다 강한 이유가 실제로 드러났다 — grep 은 우리가 아는 형태의 참조만 세지만, 지우고 빌드하면 **어떤 형태의 의존이든 링커가 말해 준다.**

> **첫 판이 틀렸다.** `git worktree add --detach HEAD` 를 썼는데 그것은 **커밋된 HEAD** 를 꺼내므로 작업 트리 변경을 보지 못한다 — 일부러 SDK 에 ADK include 를 심고 돌렸더니 **OK 를 냈다.** 추적 파일 복사 방식으로 바꾸자 같은 plant 를 잡았다. **게이트가 무엇을 보는지 확인하지 않으면 통과가 아무 뜻도 없다.**
...
#### 3-J-실측 — **세 구현이 같은 무한 루프를 갖고 있다** `[2026-08-03]`

중복을 세다 발견했다. `posix`·`ios`·`windows` 세 파일의 송신 대기 루프가 **글자까지 같고**, 둘이 잘못돼 있다.

| # | 결함 |
|---|---|
| 1 | **탈출구가 "쓰기 가능" 하나뿐**이다. `select` 가 -1 을 내도 루프가 돈다 — 다른 스레드가 송신 도중 소켓을 닫으면 `EBADF` 가 계속 나오고 그 스레드는 돌아오지 않는다 |
| 2 | **POSIX `select` 는 `tv` 를 제자리에서 깎는다.** 루프 밖에서 한 번만 세우면 첫 타임아웃 뒤 `{0,0}` 이 되고 그때부터는 대기 없는 busy spin 이다 |

**재현하지 못했다 — 그대로 적는다.** 읽지 않는 피어를 세워 송신 버퍼를 채우는 케이스를 썼으나 스핀이 나오지 않는다. 소켓이 non-blocking 이라 버퍼가 차면 `send` 가 `EAGAIN` 으로 먼저 실패하고 대기 루프의 타임아웃 경로에 닿지 않는다(고치기 전 코드로 되돌려 확인). `select` 가 -1 을 내는 경로는 공개 API 로 주입할 수 없다.

**따라서 이 수정의 근거는 실행이 아니라 코드 읽기다.** 남긴 케이스 3건은 스핀 재현이 아니라 **경계 보증**(송신이 유한 시간에 돌아온다)이다.

> `ios` 가 `posix` 와 76줄(52%) 겹치고 실제 플랫폼 차이는 **`SO_NOSIGPIPE` 하나**뿐이다(나머지는 클래스 이름과 `fprintf` 디버그 출력). 한 벌로 합칠 수 있으나 **iOS 를 빌드할 수 없어 판정 수단이 없다** — 합치기는 Xcode 가 있는 곳에서 한다. 지금 합치면 "컴파일되는지 모르는 코드" 가 된다.
