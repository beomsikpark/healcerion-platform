...
### 0.1 플랫폼 — 출시 우선순위와 개발 플랫폼은 다른 축이다

> **⚑ 출시 우선순위 확정(2026-08-02)** — **Windows 최우선 · Android·iOS 그다음 · Linux 는 개발용 · macOS 는 출시 대상 아님.** 이전 판은 개발 편의(Linux 주 개발)를 출시 우선순위처럼 서술했다. **두 축은 다르다.**

| 축 | 플랫폼 | 역할 |
|---|---|---|
| **출시 1순위** | **Windows** | 제품이 먼저 나가는 곳. 판정·검증이 여기 몰린다 |
| **출시 2순위** | **Android · iOS** | 출시 대상. Windows 뒤 |
| **개발·CI** | **Linux** | 일상 개발·회귀 판정이 도는 곳. **출시 대상이 아니다** |
| **제외** | macOS | `moana` 출하 3타깃(Android·iOS·Windows)에 없다. `arm64` 전용 제약도 있다 |

**Linux 1급화는 유지된다** — 다만 이유가 *"제품 지원 대상"* 이 아니라 **"CI 와 헤드리스 회귀 판정이 도는 곳"** 이다([goal.md §5.3](../goal.md) 의 판정 시험 ②가 여기서 돈다). Phase [0-L](./phase0-build-reproducibility.md)(`platforms/linux`)의 근거가 그것이고, **오디오(L-3) 같은 제품 기능은 출시 대상이 아니므로 우선순위가 내려간다.**
...
| 2-E | **멀티플랫폼 빌드 매트릭스 자동화** — Phase 0 의 단일 진입점 위에서 병렬 빌드, 각 산출물을 표준 경로에 수집. **순서는 §0.1 의 출시 우선순위**(Windows → Android·iOS)를 따르고, **Linux 는 CI 기본 타깃**으로 항상 돈다. **macOS 는 매트릭스에서 뺀다**(출시 대상 아님) |
