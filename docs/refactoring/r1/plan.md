...
### 3.1b 축 `X` — 이 계획이 전제하지 못하는 것 (2026-08-02 추가)

**코드를 실제로 읽어 확인한 동작 결함이 계층 합계 60건**이다(여기에 `moana` 의 SDK 대응 계층 5건이 더 있다). 이 계획은 §8 에서 *"도메인 로직은 다시 쓰지 않는다"* 고 못박는데, 그 전제는 **"지금 코드는 동작한다"** 였다. 그 전제가 성립하지 않는 지점이 있다.
...
1. **§8 의 "기존 동작 보존"이 어느 기준선을 뜻하는지 갈린다** — ADK 의 현재 동작을 보존하면 `moana` 대비 회귀 18건을 함께 고정한다
