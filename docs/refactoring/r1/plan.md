...
**결론**: belle-fw(r2)와 달리 승격할 기존 테스트·CI·배포 자산이 사실상 없다. Phase 1(회귀 기준선)·Phase 2(배포 자동화)는 "있는 것을 잇는" 작업이 아니라 **처음부터 짓는 작업**이다. 그리고 아키텍처 경계(SDK/ADK·렌더링) 뿐 아니라 **파일·함수 단위 정리 대상도 실재한다** — Phase 0·3·4 에 반영했다.

## 2. 목표 구조

### 2.0 아키텍처 결정 — feature-first clean architecture

**명시적으로 채택한다.** 지금까지 이 문서는 "인터페이스 뒤로", "책임 축 확정" 처럼 메커니즘을 서술 없이 남겨 뒀다 — 이름이 없으면 강제 장치(테스트·CI)의 판정 대상도 만들어지지 않는다. [legacy/r1](../legacy/r1/)(moana, 포트 패턴)·[r2](../r2/plan.md)(belle-fw, vtable→인터페이스 승격 + ADR-003·004)는 이미 각자의 이름과 근거를 문서에 박아 뒀고, r1(이 문서)만 그게 없었다. **선례는 [precedent-cctv.md §2.4](../legacy/precedent-cctv.md)** — `features/{domain,ports,data,application,interfaces}` + `platforms/`(HW 어댑터) + `app/`(composition root), `make test-architecture` 로 CI 강제.

**적용 범위 — SDK features·ADK features 한정.** `sdk/features/*`·`adk/features/*`(아래 §2.2) 각 모듈에 적용한다. **범위 밖**: `wrapper/`(5개 언어 바인딩, feature-first 바깥의 C ABI 소비자, Phase5) · `sample/` · `core/`(개명 전 `common/`, 공유 인프라·엔티티, feature 아님) · `sonex-app`(별도 저장소, [§7](#7-이-문서가-다루지-않는-것)) · `belle-fw`(r2 가 이미 자체 적용 중).
...
sonex-framework/
  sdk/
    ...
    platform/                 기존 유지. ports 의 구현체 — 렌더서피스 이벤트입력 소켓 오디오
      windows/  android/  ios/  macos/  headless/   (headless = §Phase0 0-G, cctv "ubuntu24" 대응)
...
| 항목 | 내용 |
|---|---|
| **0-0** | **저장소 재배치** — `client/legacy/sonex-framework`(읽기전용 미러)를 `client/sonex-framework`(쓰기 가능 작업 사본)로 복제한다. **fork base = `master`**(위 실측 기준대로 최신 tip 이고 주 개발선). 착수 직전 **재fetch 로 `origin/master` 갱신 확인** — 마지막 fetch 가 2026-07-27 이라 그 이후 원격 변화는 미확인이다. `feature-apply_v1.23.4` 의 SRI 필터 2커밋을 먼저 흡수할지는 착수 시 판단(→ 0-J). **이 시점부터 이하 모든 Phase 를 실제로 실행할 수 있다.** 힐세리온 원본에 대한 반영 방식(fork-and-PR·브랜치 위임 등)은 착수 시점에 별도 협의 |
...
| 0-F | **빌드 진입점 통일** — MSBuild(`.vcxproj` 29개)·`ndk-build`·Xcode·CMake **4갈래**에 단일 진입점 |
...
| 0-J | **`feature-apply_v1.23.4` 2커밋 흡수 판단** — master 밖 유일한 diverge 이고 내용이 SRI 필터(`HCSRIv23_3`·`HCSRIv23_4`) SDK 통합이다. **`ImageFilter` 를 건드리므로 흡수 여부를 먼저 정해야** 이후 Phase 가 어느 코드 위에 서는지 확정된다. 힐세리온에 이 브랜치의 머지 예정 여부 질의 |
...
| 평문 HTTP·멀티테넌시(`sdi` 스키마) | [goal.md §6](../goal.md) 남은 질의. ADK 백엔드 쪽 사안이라 범위 밖 |
...
**왜**: [belle-fw(r2)](../r2/plan.md) 가 세운 원칙과 같다 — 검증 인프라(Phase 1)가 서기 전에는 도메인 로직을 다시 쓰지 않는다. 그래야 "바뀐 게 맞는지"를 diff 로 판정할 수 있다. **알고리즘 자체를 다시 설계하고 싶다면 이 계획의 범위 밖이고, 별도 논제가 필요하다.**
