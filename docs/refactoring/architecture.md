...
| 에뮬레이터 | `platform/sim` 과 독립 시뮬레이터 두 갈래 | 더미 골격만([emulator-e2e.md §3.1](emulator-e2e.md)) |
...
현재는 이 정의가 **7개 코드베이스에 복제**돼 있고 선언만 3벌이다.

## 6. 이 구조가 열어주는 것

### 6.1 개발 PC 에서 장비를 돌릴 수 있다

`core/`·`features/` 가 하드웨어를 모르므로 `platform/sim/` 을 끼우면 **PC 에서 그대로 실행된다.** 앱이 `127.0.0.1:1234` 로 붙어 **스캔·측정·DICOM 전 경로가 실장비 없이 검증된다.**

**`FpgaPort` 의 골격은 이미 있다** — `lib/fpga.cpp` 가 함수 포인터 vtable 로 `DEVICE_EBI`/`DEVICE_DUMMY` 를 분기한다. 다만 `sonon.cpp:3189` 이 `DEVICE_EBI` 를 상수로 넘겨 **고를 수 없고**, 더미가 램프 패턴만 낸다. 이 절의 작업은 새 인터페이스 설계가 아니라 **있는 vtable 의 승격 + 녹화 재생**이다.
...
| 2 | **독립 시뮬레이터**(경로 A) — 정본을 첫 소비 | **앱 E2E 가 실장비 없이 돈다** |
| 3 | 장비에 **`core/ports` 도입** — `FpgaPort` 로 `lib/` 의 vtable 을 승격 | 도메인이 하드웨어에서 분리 |
| 4 | **`platform/sim` + 녹화 재생**(경로 B) | **펌웨어가 개발 PC 에서 실행** |
...
1·2 는 빌드 재현을 기다리지 않는다(§1 의 정본 계층만 있으면 된다). 3·4 는 [assessment.md §3](assessment.md) 의 Buildroot 도입과 맞물린다. CI 는 2번부터 계속 따라붙는다 — [emulator-e2e.md §6](emulator-e2e.md).
