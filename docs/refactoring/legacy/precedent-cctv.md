...
> **이 문서의 역할**: 이 갈래가 만들려는 것은 "이렇게 하면 좋아진다" 는 예측이 아니라 **이미 한 번 한 일**이다. 같은 팀이 같은 방법으로 **동종 시스템**(임베디드 장비 펌웨어 + 멀티플랫폼 클라이언트 + 클라우드)을 리팩토링했고, 그 결과가 저장소에 남아 있다.
>
...
| 시작 시점 구조 | 기술 역할별 대형 파일 | **동일** ([why.md §5.1](why.md)) |
...
**healcerion 의 HC 프로토콜이 저장소 9곳에 흩어져 있는 것과 같은 패턴이다**([why.md §2](why.md)). 리팩토링 후에는 `core/` 로 들어가 앱마다 한 벌만 남았다.
...
healcerion 현재와 대조하면 이것이 [why.md §6 시나리오 C](why.md) 가 말하는 것의 실물이다.
...
|---|---|---|---|
| **규제** | 없음 | **의료기기** — IEC 62304 / ISO 14971, 인증 유지 | **가장 큰 차이.** 변경마다 검증 산출물이 따라와야 한다. 다만 CI·추적성이 그 산출물을 만든다([principles.md §10](principles.md)) |
| **소유권** | 벤더 원본을 인수해 우리가 소유 | **힐세리온 소유**, 우리는 read-only 미러 | 작업 방식·반입 승인·IP 경계를 먼저 합의해야 한다 |
| **동시 진행 리팩토링** | 없었음 | **sonex 전환이 진행 중** | 그 전환이 3년 2개월째 도달하지 못했다([why.md §3.2](why.md)) |
