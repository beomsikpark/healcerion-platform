...
**순위가 뒤바뀐 적이 없었다.** 2023년 개발계획서가 신규 개발 사유 3개를 적었는데, 그중 하나가 정확히 목적 1이다.

> **⚑ 2026-08-02 — 실행 순서가 뒤집혔다(목적 자체는 유지).** 출시 대상이 **500C·500P** 로 확정되고 UI 를 **`moana` Qt 이관**([r2](r2/plan.md))으로 정하면서, **이번 실행 범위는 목적 2(제품 출시)가 앞선다.**
>
> | | |
> |---|---|
> | **이번 범위** | **Qt/C++ 소비자 1종.** 제품(`moana` UI)이 소비하는 것만 완성한다 |
> | **연기** | C#·Python·Flutter·JNI·ObjC++ **5종** — 폐기가 아니라 **연기**다(§5.2) |
> | **목적 1은 살아 있다** | *"외부 고객사 SDK·ADK 제공"* 은 `[CTO]` 최우선 목적이자 계획서 신규 개발 사유다. **범위에서 뺀 것이지 취소한 것이 아니다** |
>
> **연기가 안전한 이유는 순서에 있다.** [Phase 4](r1/phase4-render-boundary.md)(렌더 경계)가 서면 나머지 언어는 *"모든 UI 프레임워크가 이미 갖고 있는 표준 이미지 경로에 얹는 일"* 로 줄어든다. **경계를 제대로 세워 두면 나중에 언어를 늘리는 비용이 낮고, 지금 Qt 1벌만 해도 그 투자가 버려지지 않는다.** 순서를 뒤집어 wrapper 를 먼저 만들면 그때 결합이 언어 수만큼 곱해진다([Phase 5 선행 근거](r1/phase5-language-wrappers.md)).
...
| 단위 | 수 |
|---|---|
| 샘플 | **이번 범위 = Qt/C++ 1벌**(SDK 섹션 + ADK 섹션). 전체 목표는 언어당 1벌 = 6벌이며 나머지 5벌은 **연기**(§1 ⚑) |
...
### 5.2 지원 언어 — 이번 범위는 C++ / Qt 1종

> **⚑ 2026-08-02**: 아래 표는 **전체 목표**다. **이번 실행 범위는 `C++`(Qt6 표시 컴포넌트) 한 종**이고 나머지 5종은 연기다(§1 ⚑). 표를 지우지 않는 이유는 **연기와 폐기가 다르기 때문**이며, 각 언어의 현재 바인딩 실측은 재개 시점에 그대로 쓰인다.
