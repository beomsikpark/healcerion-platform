...
| 위험 | 영향 | 대응 |
|---|---|---|
| **NextSRI/SRI 가 CVIE 와 화질이 다르다** | 제품 품질 저하. 규제 재검증 | **가장 먼저 판정한다** — 코드 작업(B-2·B-3)보다 **화질 비교(3.5)를 앞세운다.** 등가가 아니면 **라이선스 제약과 제품 품질이 충돌**하는 것이므로 그 사실을 그대로 올린다. 우리가 정할 사안이 아니다 |
