...
### Step 2-T. 어댑터 단위테스트 — **같은 커밋에 낸다**

**[plan.md §2.6](./plan.md) 이 정한 셋이 전부 이 phase 에서 코드가 생긴다.** 어댑터만 내고 테스트를 안 내면 나중에 안 쓴다([r1 4-A1a](../r1/phase4-render-boundary.md) 와 같은 강제).

| # | 작업 |
|---|---|
| T-1 | **`moana.pro` 의 `SUBDIRS` 에 `test` 추가** — `test.depends = framework` 는 이미 선언돼 있다([plan.md §2.6.1](./plan.md)). `QT += testlib` |
| T-2 | **좌표·단위 변환**(§1.5 ②) — `scanLine`↔`linePosition` 왕복 항등성 · 경계값(0·1·최대 scanline). **틀리면 측정값이 틀리므로 이 계획에서 가장 중요한 테스트다** |
| T-3 | **명령 매핑 전수 대조**(§1.2·§1.3) — 죽은 명령 제외 후 **매핑 누락 0건**을 테스트가 판정한다. 통짜→개별 분해에서 빠진 필드도 여기서 잡는다 |
| T-4 | **프레임 어댑터 필드 대응**(§1.5) — 왕복 + **사이드카 필드 보존**(`fl2`·`multiFocus*`·`presetName` 등 ⑤) |
| T-4b | **상태 어댑터**([plan.md §2.6.3 ④](./plan.md)) — `ScanContext` 397개소가 넘어간 뒤의 조회·설정 경로. **필드 범주별 read/write 왕복**(스캔 파라미터·영상처리·B·모드 구조체) |
| T-5 | **변환 로직을 순수 함수로 유지한다**([plan.md §2.4 A-5](./plan.md)) — Qt 객체·전역 상태 의존 시 테스트가 붙지 않는다 |
| T-6 | **CI 에 연결** — `moana` 는 CI **0건**이라 이것이 첫 CI 다. 빌드 게이트 + 단위테스트가 최소 구성 |
| **T-7** | **mock 장치 서버로 배선을 판정한다** — [plan.md §2.5.3 ②](./plan.md) 의 실질 안전망이다. 단위테스트(T-3)가 **매핑 테이블**을 대조한다면, 이것은 **앱이 실제로 보낸 것**을 서버가 받아 기록·검증한다. **둘은 다르다** — 매핑이 맞아도 호출부가 안 부르면 테이블 대조는 통과한다 |
| T-7a | 시나리오 — 연결 → 스캔 시작 → **모드별 파라미터 전 필드 변경** → 정지. 통짜→개별 분해(§1.2)에서 빠진 필드가 여기서 드러난다 |
| T-7b | **`moana` 원본과 대조한다** — 같은 UI 조작에 대해 이관 전 `moana` 가 보내던 명령열과 이관 후 보내는 명령열을 mock 서버가 각각 기록해 비교. **이것이 이 계획에서 유일하게 "이관 전후 동등성" 을 자동 판정하는 지점이다** |
...
| **3.0b** | **배선 판정** | mock 장치 서버(T-7) — 이관 전후 명령열 대조 | **차이 0.** 실장비 없이 도는 유일한 동등성 판정 |
...
| **3.10** | **단위테스트** | `make test`(신설) — 좌표 변환·명령 매핑·프레임 어댑터 | **CI 에서 통과.** `moana` 최초의 자동 판정이다 |
| 3.11 | 명령 매핑 커버리지 | T-3 전수 대조 | 매핑 누락 **0건** |
