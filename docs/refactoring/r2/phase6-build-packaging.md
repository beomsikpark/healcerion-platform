...
`moana` 출하 3타깃(Android·iOS·Windows 데스크톱)과 SDK 지원 4종(Windows·Android·iOS·macOS)의 교집합에서 정한다. **힐세리온 결정 사항**이며([plan.md §6](./plan.md)) 이 phase 의 범위를 직접 정한다.
...
| **출시 플랫폼 미확정** | 범위가 정해지지 않는다 | E-1 — 힐세리온 결정. **정해지기 전에는 플랫폼별 작업을 벌이지 않는다** |
...
- [plan.md](./plan.md) §0.4(라이선스 대체 대상)·§3 Phase 6·§6(출시 플랫폼 미확정)
