...
### Step 4-T. 데이터 계층 단위테스트

**[plan.md §2.6.3 ⑤](./plan.md).** DDL 이 동일하므로(§1.1) **대조가 명확하다** — 값이 그대로 왕복하면 된다.

| # | 작업 |
|---|---|
| T-1 | **9테이블 값 왕복** — 쓰기 → 읽기 → 원본 대조. 테이블 단위로 끊어 A-1 진행에 맞춘다 |
| T-2 | **`INSERT OR REPLACE` 동작 대조**(§1.3 ⑤) — `moana` 구현과 ADK 구현이 같은 결과를 내는지. **다르면 회귀이므로 이 phase 소관이다** |
| T-3 | **경계 케이스** — 빈 문자열·NULL·유니코드 환자명·긴 UID. `moana` 가 질의를 **문자열 조립**으로 만들므로(§1.3 ②) 값이 SQL 문법에 섞이는 입력을 반드시 넣는다 |
| T-4 | **위치 기반 컬럼 매핑 검증**(§1.3 ③) — 컬럼 순서가 바뀌면 조용히 어긋난다. **성공/실패가 아니라 값까지 대조** |
...
| **3.12** | **데이터 단위테스트** | Step 4-T | **CI 통과.** 값 왕복·경계 케이스 포함 |
