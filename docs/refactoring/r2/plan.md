...
#### 2.6.2 붙일 곳 셋 — 위험이 집중되고 순수 함수인 것

**전면 단위테스트는 4주에 들어가지 않는다.** 그래서 **틀리면 조용히 나가고 사람이 화면으로 못 잡는 것**에만 붙인다.

| # | 대상 | 왜 여기인가 | 판정 형태 |
|---|---|---|---|
| **①** | **좌표·단위 변환** — `scanLine`(절대 index) ↔ `linePosition`(0~1 비율)([§1.5 ②](#15-sononframe--hcstreamdata--이름-매핑이-아니라-의미-변경이-섞여-있다)) · `ppcm` 대체분 | **틀리면 측정값이 틀린다.** 임상 영향이고, 화면으로는 몇 픽셀 차이를 못 본다 | 왕복 변환 항등성 · 경계값(0·1·최대 scanline) |
| **②** | **명령 매핑** — `SONON_CMD_*` 63종 → SDK request([Phase 2-A](./phase2-sdk-adk-adapter.md)) | **통짜 구조체를 개별 request 로 분해**하는 작업이라 필드가 조용히 빠진다 | **매핑 테이블 전수 대조** — 죽은 명령 제외 후 **누락 0건**을 테스트가 판정 |
| **③** | **프레임 어댑터 필드 대응** — `SononFrame` ↔ `StreamData`, 사이드카 필드 포함([Phase 2-C](./phase2-sdk-adk-adapter.md)) | 자리가 없는 필드(`fl2`·`multiFocus*`·`presetName`)가 조용히 누락된다 | 필드 왕복 · 사이드카 보존 |

**셋 다 순수 변환 로직이라 GL 컨텍스트도 장비도 필요 없다.** 그래서 **CI 에서 돈다** — 이 계획에서 자동으로 돌릴 수 있는 유일한 테스트다.

#### 2.6.3 안 붙일 곳

| 대상 | 이유 |
|---|---|
| **QML·UI** | UI 를 바꾸지 않는 것이 이 계획의 전제다([§2.3](#23-무엇이-남고-무엇이-사라지는가)). 안 바꾸는 것에 테스트를 새로 붙이는 것은 4주에 들어가지 않는다 |
| **SDK 내부**(렌더·필터·측정 계산) | **[r1](../r1/plan.md) 소관.** r2 는 소비자다([§2.5.3](#253-그래서-판정을-셋으로-가른다)) |
| DB 스키마 | DDL 이 이미 동일하므로([Phase 4 §1.1](./phase4-data-layer.md)) 값 왕복 확인으로 충분하다 |
| 통합·E2E | 배선 판정은 mock 장치 서버가 맡는다(§2.5.3 ②). 별도 프레임워크를 들이지 않는다 |
