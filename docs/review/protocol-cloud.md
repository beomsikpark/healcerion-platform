...
### 6.1 Java 백엔드 — 핸들러 136개

관리자용(`/Admin/`)과 모바일용(`/API/`)이 컨트롤러 단위로 분리돼 있다. method-level `@RequestMapping` 실측이다.
...
> **정정 (2026-07-28)**: 초판은 SDI 관리자를 32, 총계를 124 로 적었다. `*DeviceUser`·`*BatteryUser` CRUD 12건과 헬스체크가 누락돼 있었다 — 상세·중복 라우트 결함 = [cloud-server.md §2.2](cloud-server.md).
